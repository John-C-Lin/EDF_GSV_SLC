...
# Adapt Ben Fasoli's "06_monte_carlo_source.r" from landfill/CH4 case to Inland Port2 polygon case
...
# JCL:  subset of Inland Port2 area
source_bbox <- extent(-111.9771, -111.954, 40.741, 40.749)
...
# JCL:  observations covering Inland Port2 polygon
# Area observations excluding line source
# observations_bbox <- extent(-112.09, -112.01, 40.715, 40.775)
observations_bbox <- extent(-111.9844, -111.952, 40.74, 40.75)
observations <- read_feather("gsv-data/by_receptor/by_receptor.feather") %>%
    filter(
        # !is.na(ch4d_ppm_ex),
        longitude >= observations_bbox@xmin,
...
if(regenerate.footTF){
  ...
  saveRDS(footprint_full_domain,"footprint_full_domain_inlandport2.rds")
} else {
  print("reading in footprint_full_domain_inlandport2.rds from previous run")
  footprint_full_domain <- readRDS("footprint_full_domain_inlandport2.rds")
} # if(regenerate.footTF){
...
png(paste0("img/monte_carlo/source_r_inlandport2_",tracer,".png"), width = 2000, height = 2000, res = 300)
require("fields")
...
png("img/monte_carlo/density_inlandport2.png", width = 2000, height = 2000, res = 300)
h <- hist(r, breaks = 25)
...
ggplot(df, aes(x = predicted, y = measured)) +
    ...
    labs(
        ...
        y = tracer,
        title = "Inland Port2 observations vs STILT",
        subtitle = bquote("Flux" == .(round(mod$coefficients[2], 1)) ~ frac(mu * mol, m^2 ~ s) ~
        "    " ~ r^2 == .(round(summary(mod)$r.squared, 3)))
    )
ggsave("img/monte_carlo/source-binary-scatterplot_inlandport2.png", width = 6, height = 5)
...
ggsave("img/monte_carlo/source-optimized-scatterplot_inlandport2.png", width = 6, height = 5)
