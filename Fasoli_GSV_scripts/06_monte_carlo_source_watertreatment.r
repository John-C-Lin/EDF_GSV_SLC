...
regenerate.footTF <- TRUE    # whether or not to regenerate all footprints (TRUE), or load from previous run (FALSE)
#####################
...
source_bbox <- extent(-111.94, -111.928, 40.80, 40.823)   # source area around water treatment plant in northern SLC (https://www.slc.gov/utilities/wastewater-pretreatment/)
...
# Area observations 
#observations_bbox <- extent(-112.09, -112.01, 40.715, 40.775)
observations_bbox <- extent(-111.946, -111.927, 40.798, 40.83)
observations <- read_feather("gsv-data/by_receptor/by_receptor.feather") %>%
    filter(
        longitude >= observations_bbox@xmin,
        longitude <= observations_bbox@xmax,
...
if(regenerate.footTF){
  ...
  saveRDS(footprint_full_domain,"footprint_full_domain_watertreatment.rds")
} else {
  print("reading in footprint_full_domain_landfill.rds from previous run")
  footprint_full_domain <- readRDS("footprint_full_domain_watertreatment.rds")
} # if(regenerate.footTF){
...
ggplot(df, aes(x = predicted, y = measured)) +
    ...
    labs(
        ...
        y = bquote(CH[4 * e] ~ " ppm"),
        title = "Water Treatment Plant observations vs STILT",
        subtitle = bquote("Flux" == .(round(mod$coefficients[2], 1)) ~ frac(mu * mol, m^2 ~ s) ~
        "    " ~ r^2 == .(round(summary(mod)$r.squared, 3)))
    )
ggsave("img/monte_carlo/source-binary-scatterplot_watertreatment.png", width = 6, height = 5)
...
ggsave("img/monte_carlo/source-optimized-scatterplot_watertreatment.png", width = 6, height = 5)
