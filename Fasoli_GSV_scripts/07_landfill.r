...
# gridded <- read_feather("gsv-data/by_receptor/by_receptor.feather")
# bbox <- extent(-112.075, -112.025, 40.741, 40.741)


# gridded_subdomain <- gridded %>%
#     mutate(
...
#     mutate(job_id = paste(
#         sep = "_",
#         strftime(time, format = "%Y%m%d%H", tz = "UTC"),
#         round(longitude, 3),
#         round(latitude, 3),
#         "3"
#     ))
...
# job_ids <- gridded_subdomain$job_id
# filenames <- paste0(job_ids, ".nc")
# filenames_fetch <- setdiff(filenames, dir("footprints/landfill"))

# # Fetch footprint files that don't exist locally
# i <- 1
# page_size <- 100
# len <- length(filenames_fetch)
# repeat {
#     if (i >= len) break
#     end <- min(length(filenames_fetch), i + page_size - 1)
#     page_filenames <- filenames_fetch[i:end]

#     stdin <- paste(file.path("gs://gsv-x-storage/stilt/nc", page_filenames),
#         collapse = "\n"
#     )
#     cmd <- glue(
#         'echo "{stdin}" ',
#         "| /usr/local/google-cloud-sdk/bin/gsutil ",
#         "-m cp -I footprints/landfill/"
#     )
#     system(as.character(cmd))
...
# # filepaths <- file.path('footprints', 'gravelpit', filenames)
# # rasters <- filepaths[file.exists(filepaths)] %>%
# #   lapply(function(f) sum(brick(f))) %>%
# #   brick() %>%
# #   readAll() %>%
# #   setZ(job_ids[file.exists(filepaths)], name = 'job_id')
# # saveRDS(rasters, 'footprints/gravelpit.rds')
# # rasters <- readRDS('footprints/gravelpit.rds')


# # source_locations <- bind_rows(list(
# #   expand.grid(longitude = seq_002(-111.913, -111.911),
# #               latitude = seq_002(40.819, 40.831)),
# #   expand.grid(longitude = seq_002(-111.915, -111.911),
# #               latitude = seq_002(40.815, 40.817)),
# #   expand.grid(longitude = seq_002(-111.917, -111.911),
# #               latitude = seq_002(40.809, 40.813)),
# #   expand.grid(longitude = seq_002(-111.915, -111.907),
# #               latitude = 40.807),
# #   expand.grid(longitude = seq_002(-111.913, -111.907),
# #               latitude = 40.805),
# #   expand.grid(longitude = seq_002(-111.911, -111.907),
# #               latitude = 40.803),
# #   data.frame(longitude = -111.909,
# #              latitude = 40.801)
# # ))
# # extent(-112.053, -112.035, 40.743, 40.749)
...
# #
# #
# # gridded_subdomain_with_footprint <- gridded_subdomain %>%
# #   inner_join(source_footprint_totals, by = 'job_id')
# # write.csv(gridded_subdomain_with_footprint,
# #           'gridded_subdomain_with_footprint.csv', row.names = F)
# #
# #
...
# # # )
# #
# # gravel_pit_receptors <- unique(gridded_subdomain[c('longitude', 'latitude')])
# #
...
# #
# #
# # case_study_dates <- c(local_time('2019-07-25'),
...
# #
# # job_ids <- getZ(rasters)
# # job_index <- which(job_ids == job_id)
# #
# #
...
# #
# # with(gridded_subdomain_with_footprint_case_studies,
# #      plot(gravelpit_footprint, pm25_ugm3_ex))
# # abline(mod, col = 'red')
# #
